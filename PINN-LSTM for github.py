...
class PopulationNet(nn.Module):
    ...
    def forward(self, a_nd, t_nd):
        x = torch.cat([a_nd, t_nd], dim=1)  # [N, 2]
        x = x.unsqueeze(1)  # [N, 1, 2] 作为长度为1的序列
        out, _ = self.lstm(x)  # out: [N, 1, hidden_size]
        last_out = out[:, -1, :]  # 取最后一步 [N, hidden_size]
        p = self.output_layer(last_out)  # [N, 1]
        return p * (1 - a_nd)
def sample_points():
...
with open('training_report.txt', 'w', encoding='utf-8') as f:
    for name in scenarios:
        ...
        f.write("\n\n")
print("training_report.txt is finished")
